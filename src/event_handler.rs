...
impl EventHandler {
    pub fn handle(mut state: &mut State, event: Event<()>, control_flow: &mut ControlFlow) {
        match event {
            Event::WindowEvent { ref event, window_id }  if window_id == state.window().id() => {
                Self::handle_window(&mut state, control_flow, event);
            }
    ...
    fn handle_window(state: &mut State, control_flow: &mut ControlFlow, event: &WindowEvent) {
        match event {
            WindowEvent::CloseRequested => *control_flow = ControlFlow::Exit,
            WindowEvent::KeyboardInput { input, .. } => Self::handle_keyboard(state, control_flow, input),
            WindowEvent::MouseWheel { delta, phase: TouchPhase::Moved, .. } => Self::handle_mouse_scroll(state, delta),
            WindowEvent::MouseInput { button, state: ElementState::Pressed, .. } => Self::handle_mouse_pressed(state, button),
    ...
    fn handle_keyboard(state: &mut State, control_flow: &mut ControlFlow, input: &KeyboardInput) {
